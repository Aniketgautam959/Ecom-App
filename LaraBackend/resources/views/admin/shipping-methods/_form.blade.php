@php
    $method = $method ?? null;
    $val = fn ($field, $default = null) => old($field, $method ? $method->{$field} : $default);
@endphp
...
<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" name="price" value="{{ $val('price', 0) }}" class="form-control @error('price') is-invalid @enderror" required>
        @error('price')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-md-4">
        <label class="form-label">Free Threshold</label>
        <input type="number" step="0.01" name="free_threshold" value="{{ $val('free_threshold') }}" class="form-control @error('free_threshold') is-invalid @enderror">
        @error('free_threshold')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
